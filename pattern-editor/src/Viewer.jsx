...
export default class Viewer extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.rows !== this.props.rows) {
      ...
      this.camera.lookAt(
        (this.width * this.stitch_width) / 2,
        0,
        (this.height * this.stitch_height) / 2
      );
    }
  ...
  componentDidMount() {
    ...
    let altFloorMaterial = new THREE.MeshBasicMaterial({
      color: 0x419c03,
      side: THREE.DoubleSide,
    });
    ...
    function generateColouredTexture(colourBitmap) {
      ...
      let colourMap = new THREE.DataTexture(
        pixelData,
        that.width,
        that.height,
        format,
    ...
    function generateStripedTexture(colourBitmap) {
      ...
      console.log(
        "size is " + size + " with " + that.width + " " + that.height
      );
      var pixelData = new Uint8Array(3 * size);
      for (var i = 0, len = size; i < len; i++) {
        var i3 = i * 3;
    ...
    let bitmap = [...Array(this.height)].map((x) =>
      [...Array(this.width)].map((f) => false)
    );
    bitmap[10][5] = true;
    bitmap[11][5] = true;
...
`;
    ...
    this.replaceColourTexture = (newBitmap) => {
      ...
      cube.material.uniforms.xRange.value = this.width * this.stitch_width;
      cube.material.uniforms.zRange.value = this.height * this.stitch_height;
      cube.material.uniforms.colourMap.value = generateColouredTexture(
        newBitmap
      );
      cube.needsUpdate = true;
    };
    ...
    this.width = 10;
    this.height = 10;
    let newBitmap = [...Array(this.height)].map((x) =>
      [...Array(this.width)].map((r) => false)
    );
    // newBitmap[5][8] = true;
    this.replaceMesh(newBitmap);
    let newColourTexture = [...Array(this.height)].map((x) =>
      [...Array(this.width)].map((r) => "fff")
    );
    ...
    function from01Bitmap(bitmap) {
      ...
      let map = lines.map((str) =>
        str.split("").map((v) => (v == "0" ? false : true))
      );
      console.log(map);
      return map;
    }
...
00000000000000000000000000000000`;
      ...
      let map = lines.map((str) =>
        str.split("").map((v) => (v == "0" ? "#de1e7e" : "#F0FEAF"))
      );
